...
int main(int argc, char *argv[]) {	
	...
	while (std::getline(file, line)) {
		...
		// Handle input date before first data date
		std::string earliestDate = bitcoinExchange.getEarliestDate();
		if (date < earliestDate) {
			std::cout << "Error: date is before earliest data => " << date << std::endl;
			continue;
		}
